...
const getToken = async(tenant) => {
    const certificado = fs.readFileSync(tenant.gnCertificado);
    const credenciais = {
        client_id: tenant.gnClientId,
        client_secret: tenant.gnClientSecret,
    }
...
const createWebhook = async(tenants) => {
    for (const tenant of tenants) {
        ...
        const result = await axios(config);
        console.log('Webhook de ', tenant.slug,'  criada com sucesso!');
        return result.data;
    }
}
